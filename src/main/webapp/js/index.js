 		...
 		$http.jsonp(url)
 	     .success(
 	    		 function(response) {
 	    			...
 	    			this.emailInfo = response.feed.entry;
 	    			controllerRef.userEmails = [];
 	    			for (i = 0; i < this.emailInfo.length; i++) { 
 	    			   console.log( (i+1) + "EMAIL :" + this.emailInfo[i].gd$email[0].address);
 	    			  controllerRef.userEmails[i] = this.emailInfo[i].gd$email[0].address;
 	    			}
